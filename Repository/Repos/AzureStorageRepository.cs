...
public class AzureStorageRepository : IAzureStorageRepository
{
    ...
    public AzureStorageRepository(IConfiguration configuration)
    {
        ...
        _storageConnectionString = configuration.GetValue<string>("AzureStorage");
    }
    public async Task<BlobResponse?> UpdateFileAsync(IFormFile? file, string? fileName, string containerName, string? fileExtension, bool isPrivate)
    {
